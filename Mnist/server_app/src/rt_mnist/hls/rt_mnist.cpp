...
void read_image(uint32 ram[N_ROWS*N_COLS*BATCH_SZ/2], fixed16 image[BATCH_SZ][N_ROWS][N_COLS]){
	// Read the input stream and put the values in the image array
...
#pragma HLS INLINE off
	...
	batch_rd: for(b=0; b<BATCH_SZ; b++){
		row_rd: for (r=0; r<N_ROWS; r++) {
			col_rd: for (c=0; c<N_COLS; c+=2){
#pragma HLS PIPELINE
				data_in = ram[i >> 1] & 0x0000ffff; 
				norm_data_in = (fixed16)((data_in)/(255));
				image[b][r][c] = norm_data_in;

				data_in = (ram[i >> 1] & 0xffff0000) >> 16; 
				norm_data_in = (fixed16)((data_in)/(255));

				image[b][r][c+1] = norm_data_in;
...
#pragma HLS array_partition variable=pool1_out complete dim=4
	...
	while (1)
	{
		...
		uint32 ram[BATCH_SZ*N_ROWS*N_COLS/2];
		...
		MEM_READ( payload_addr[0], ram, N_ROWS*N_COLS);
		...
		output_digit[0] = (uint32)digit;


		
		MEM_WRITE( output_digit , output_buffer_addr, 4 );

		ROS_PUBLISH(video_pubdata,video_num_out);		
	}
