...
int zed_iic_axi_init( zed_iic_t *pIIC, char szName[], uint32_t* CoreAddress )
{
   ...
   //if ( pContext == NULL )
   //{
   //   printf("Failed to allocate context data for FMC-IIC-AXI implementation\n\r" );
   //   return 0;
   //}
  ...
  do
  {
    StatusReg = XIic_Read8Bit(pContext->CoreAddress, XIIC_SR_REG_OFFSET);
    //printf("[%s] Xil_In8(pContext->CoreAddress + XIIC_SR_REG_OFFSET) => 0x%02X\n\r", pContext->szName, StatusReg );
                                         ...
                                         uint8_t *pBuffer, uint8_t ByteCount)
{
  ...
  do
  {
    ...
    if ((StatusReg & XIIC_SR_RX_FIFO_EMPTY_MASK) != XIIC_SR_RX_FIFO_EMPTY_MASK)
    {
       ...
       */
      XIic_ReadReg(pContext->CoreAddress, XIIC_DRR_REG_OFFSET/4);
    }
    ...
    {
    	ControlReg = XIic_Read8Bit(pContext->CoreAddress , XIIC_CR_REG_OFFSET);
    	ControlReg = ControlReg & 0xFB;  // Clear the MSMS bit.
    	XIic_Write8Bit(pContext->CoreAddress , XIIC_CR_REG_OFFSET, ControlReg);
    }
  } while (StatusReg != (XIIC_SR_RX_FIFO_EMPTY_MASK |
			                XIIC_SR_TX_FIFO_EMPTY_MASK));
#endif
