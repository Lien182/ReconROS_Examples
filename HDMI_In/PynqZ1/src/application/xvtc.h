...
typedef struct {
	uint16_t DeviceId;		/**< DeviceId is the unique ID of the VTC
				  *  core */
	uint32_t* BaseAddress;	/**< BaseAddress is the physical base address
				  *  of the core's registers */
} XVtc_Config;


typedef void (*XVtc_CallBack)(void *CallBackRef, uint32_t Mask);
typedef void (*XVtc_ErrorCallBack)(void *CallBackRef, uint32_t ErrorMask);
...
#define XVtc_WriteReg(BaseAddress, RegOffset, Data) \
	Out32((BaseAddress) + (RegOffset/4), (Data))
