...
void* node_thread(void * arg)
{
	...
	std_msgs__msg__UInt32 output_msg;
	...
	{
		if( ros_publisher_init(&resources_pubdata[sett->cnt], &resources_rosnode[sett->cnt], ROS_TYPE_MESSAGE_TYPE, "angle") < 0 )
		{
			ros_node_destroy(&resources_rosnode[sett->cnt]);
			return (void*)-1;
		}
		if(ros_subscriber_init(&resources_subdata[sett->cnt], &resources_rosnode[sett->cnt],ROS_TYPE_MESSAGE_TYPE,"legangle", 10) < 0)
		{
			ros_node_destroy(&resources_rosnode[sett->cnt]);
			return (void*)-1;
		}
		...
		for(i = 0; i < ITERATIONS; i++ )
		{
			...
			ros_subscriber_message_take(&resources_subdata[sett->cnt], &output_msg);
