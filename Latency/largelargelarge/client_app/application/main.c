...
void* node_thread(void * arg)
{
	...
	std_msgs__msg__UInt32MultiArray * sorted_msg = std_msgs__msg__UInt32MultiArray__create();	
	...
	{
		if( ros_publisher_init(&resources_pubdata[sett->cnt], &resources_rosnode[sett->cnt], ROS_TYPE_MESSAGE_TYPE, "unsorted") < 0 )
		{
			ros_node_destroy(&resources_rosnode[sett->cnt]);
			return (void*)-1;
		}

		ros_subscriber_init(&resources_subdata[sett->cnt], &resources_rosnode[sett->cnt],ROS_TYPE_MESSAGE_TYPE, "sorted", 0);
...
int main(int argc, char **argv) 
{
	...
	uint32_t *u32usorted = malloc(BLOCK_SIZE * 4);
	uint32_t *u32sorted  = malloc(BLOCK_SIZE * 4);
